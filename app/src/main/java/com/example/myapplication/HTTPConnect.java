...
public class HTTPConnect {
    ...
    public void readCustomerContact(String code_device, Context context){
        SharedPreferences sPref = PreferenceManager.getDefaultSharedPreferences(context);
        String token = sPref.getString(Constants.TOKEN_KEY,"").replace("%20"," ");
        String host = sPref.getString(Constants.HOST_URL,"");
        Request request = new Request.Builder()
                .addHeader("Authorization",token)
                .url("https://"+host+"/api/calls/readCustomerContact?code_device="+code_device+"&typeContact=inCall")
                .build();
        client.newCall(request).enqueue(new Callback() {
            @Override
            public void onFailure(@NotNull Call call, @NotNull IOException e) {
